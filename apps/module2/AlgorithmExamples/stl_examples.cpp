//
// File:    STL_examples.cpp
// Author:  Your Glorious Instructor
// Purpose:
// Illustrate the use of the more commonly used algorithms out of the STL's
// algorithms library using Google Test
//
...
// Examples for each STL algorithm. Verified with Google test suite. While code
// duplication is rampant, The goal is to provide self-fulfilling examples that
// can be pulled individually for others to use and look at.
...
// std::any_of: return true/false if any element of a collection causes the predicate lambda exp to return true
//
...
// 
// std::all_of: returns true/false is all elements of a collection causes the predicate lambda exp to be true.
//
...
//
// std::none_of:: return true if no element of collection causes lambda exp to be true.
//
...
//
// std::for_each: apply callable object to contents of collection
//
...
//
// std::count, std::count_if:  count number of instances of element in collection.
//
...
// 
// std::mismatch:  return first mismatching pair out of two ranges 
...
//
// std::find, std::find_if:  search for element in range
//
...
// std::copy, std::copy_n:  copy range from content.
// std::front_inserter, std::back_inserter: insert things into front or back of container
//
...
//
// std::move: move elements from range to range.
//
...
//
// std::remove, std::remove_if: remove elements out of a range.
//
...
//
// std::transform:  apply callable object to contents of range.
//
...
//
// std::replace, std::replace_if:  Replace items in collection.
//
...
//
// std::swap, std::swap_range: swap contents of a collection
//
...
//
// std::unique: Eliminates all except the first from any following group of equivalent elements in a range
//
...
TEST(pop_heap, ExampleOne) {
    ...
    const std::vector<int> popped_heap{5,5,4,4,2,3,1,6};
    EXPECT_EQ(v,popped_heap);
